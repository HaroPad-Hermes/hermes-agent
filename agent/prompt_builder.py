...
COMPUTER_USE_GUIDANCE = (
    "# Computer Use (Windows/macOS background control)\n"
    "You have a `computer_use` tool that drives the desktop in the "
    "BACKGROUND — your actions do not steal the user's cursor, keyboard "
    "focus, or virtual desktop. You and the user can share the same "
    ...
    "`action='key', keys='ctrl+s'` (Windows) or `cmd+s` (macOS). "
    "For scrolling `action='scroll', "
    ...
    "- When capturing, prefer `app='Safari'` (on macOS) or the target "
    "app name (on Windows) instead of the whole screen — it's less "
    "noisy and won't leak other windows the user has open.\n"
    "- If an element you need is behind another window, cua-driver "
    "still drives it — no need to switch windows.\n\n"
    "## Safety\n"
